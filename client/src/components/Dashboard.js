...
const client = new ApolloClient({
    link: createHttpLink({ uri: `https://users-server-crud2.herokuapp.com/graphql` }),
    cache
});
...
function PersonsList() {
    const { loading, error, data, refetch, networkStatus } = useQuery(GET_PERSONSS);
            ...
            return (
                <div key={id}>
                    ...
                    >
                        ...
                        />
                        <button className="btn" type="submit">Update person</button>
                    </form>
                    <form
                        onSubmit={e => {
                            e.preventDefault();
                    ...
                    >
                        <button className="btn materialize-red btn-flat input-field" type="submit">Delete person</button>
                    </form>
                </div>
            );
        });
